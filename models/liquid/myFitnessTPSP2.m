...
function [fitness,simOut]=myFitnessTPSP2(x,datatime,DataSet,MeanData,fillRate,ay,tao)
%% 根据变量获取参数
...
Ts=0.0005;
tspan=0:Ts:10;
[t,y]=ode45(@(t,y) DMTPfcn4(t,y,sim_param,Ts,ay),tspan,x0);
% 计算三向力
len=length(t);
Fy=zeros(len,1);
Fz=zeros(len,1);
...
%% DMTP微分方程
function dy=DMTPfcn4(t,y,sim_param,Ts,ay)
ddy=ay;
dy=zeros(12,1);
dy(1)=y(2);
dy(2)=ddy;
