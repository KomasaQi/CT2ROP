...
function [fitness,simOut]=myFitnessTP(x,datatime,DataSet,MeanData,fillRate,ay,T,tao)
%% 根据变量获取参数
...
Ts=0.0005;
tspan=0:Ts:10;
[t,y]=ode45(@(t,y) DMTPfcn2(t,y,sim_param,Ts,ay,T),tspan,x0);
% 计算三向力
len=length(t);
Fy=zeros(len,1);
Fz=zeros(len,1);
...
%% DMTP微分方程
function dy=DMTPfcn2(t,y,sim_param,Ts,ay,T)
ddy=ay*sin(2*pi/T*t);
dy=zeros(9,1);
dy(1)=y(2);
dy(2)=ddy;
